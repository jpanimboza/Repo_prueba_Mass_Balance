Graph_NonAdj <- function(att, data4model, bo_t, Nyear, bit_LM, rawData, 
                         B_LMf_w, B_LMf_idw, B_LMf_krg, e_obs_mod,temp_dir, wb){
  # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
  ...
  yl <- c(min(gg_LM$value, na.rm = T) - min(gg_LM$value, na.rm = T)%%k, 
          max(gg_LM$value, na.rm = T)+k- max(gg_LM$value, na.rm = T)%%k)
  ...
  array <- lapply(setNames(1:2, names(array)), function(i){
    array[[i]] %>% tibble::rownames_to_column("Nyear")  %>%
      pivot_longer(2:4, names_to = "data")
  }) 
  ...
  plot <- ggplot(array$B_a[array$B_a$data %in% c("B_LMf_w_a",
    ...
    scale_color_manual(breaks=c("B_LMf_w_a",
                       ...
                       labels = c("B_LMf_w_a", 
                                  "B_LMf_idw_a",
                                  "B_LMf_krg_a"),
                       values = c("brown4", "blue4", "darkgreen")) +
